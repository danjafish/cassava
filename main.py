...
CFG = {
    ...
    'verbose_step': 1,
    'log_file': "/home/samenko/Cassava/logs/baseline.log",
    'test_fold': 0,
    'fp16':True,
    'soft_labels_file': False,#'/home/samenko/Cassava/tmp/soft_labels.csv',
    'train_file': "/home/data/Cassava/train.csv",
    'save_model': False,
    'probDrop_coef': False#0.95
}
...
def train_one_model(data, soft=False):
    ...
    if CFG['soft_labels_file']:
        ...
        if CFG['probDrop_coef']:
            ...
            soft_labels['drop'] = (soft_labels['label'] != soft_labels['predic_label']) &  (soft_labels['max_prob'] > CFG['probDrop_coef'])
            print("dropped", soft_labels['drop'].sum())
            clean_soft = soft_labels[~soft_labels['drop']][['image_id','label','fold','0','1','2','3','4']].reset_index(drop=True)
            soft_labels = clean_soft
    ...
    val_data = data[data.fold == CFG['test_fold']].reset_index(drop=True)
    train_ds = CassavaDataset(train_data,TRAIN_AUGS, TEST_AUGS, mode='train' ,soft=soft)
...
def main():
    ...
    for train_index, test_index in strkf.split(data.image_id, data.label):
        ...
        f = f + 1
    train_one_model(data,soft=bool(CFG['soft_labels_file'])) 
